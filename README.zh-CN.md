...
5. **测试连接** — 点击「检查连接」探测模型
6. **检测实际协议** — 使用一个模型分别测试 `/chat/completions` 与 `/responses`，显示真实可用结果
7. **配置模型** — 筛选长列表、复制参数，逐个设置上下文窗口、最大输出、输入类型
8. **应用预置** — 从注册表自动补全或手动选择
9. **修复已知方言兼容项** — DeepSeek 兼容端点缺少历史 `reasoning_content` 回传配置时，点击「修复并应用」；正常切换模型时插件也会自动预检
10. **保存** — 重复模型 ID 会被拒绝，未保存修改会明确显示
11. **删除供应商** — 二次确认后删除配置；关联 credential 会保留，不会误删密钥
