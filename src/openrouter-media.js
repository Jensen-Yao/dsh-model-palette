...
async function executeImage(ctx, config, args, exec, allowManualOverride) {
  ...
  return { model, free_endpoint: Boolean(freeEndpoint), manual_paid_override: manualPaidOverride, files, usage: response.usage ?? null }
}
...
async function executeVideo(ctx, config, args, exec, allowManualOverride) {
  ...
  return { id: response.id, polling_url: response.polling_url, status: response.status, model, free_endpoint: free, manual_paid_override: manualPaidOverride, pricing_skus: entry.pricing_skus ?? {} }
}
