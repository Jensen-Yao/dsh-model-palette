		...
		function ConfigPanel({ api, isLoopback, t }) {
			...
			const save = async () => {
				...
				try {
					...
					const savedModelIds = new Set(profile.models.map((model) => String(model.id)));
      ...
      models:
		...
		*/
		...
		const zh = {
			...
			"relay.statusHint": "返回 401 表示已经到达供应商，应检查 credential；瞬态 socket 重置会在新连接上自动重试两次；重试后返回 503 表示上游仍不可达。502/504 或 TLS 错误可能表示请求不可重放或发生了非瞬态中继错误。",
			"relay.retryPolicy": "默认恢复策略：ECONNRESET 等瞬态连接错误会新建 HTTPS 连接重试 2 次；不超过 16 MiB 的请求体可重放。",
			"relay.copy": "复制地址",
			"relay.copied": "已复制",
		...
		const en = {
			...
			"relay.statusHint": "HTTP 401 means the request reached the provider, so check the credential. A transient socket reset is retried twice on a fresh connection; HTTP 503 after retries means the upstream is still unavailable. HTTP 502/504 or a TLS error can indicate a non-replayable or non-transient relay failure.",
			"relay.retryPolicy": "Default recovery: 2 fresh HTTPS attempts after ECONNRESET and other transient transport failures; request bodies up to 16 MiB are replayed.",
			"relay.copy": "Copy URL",
			"relay.copied": "Copied",
