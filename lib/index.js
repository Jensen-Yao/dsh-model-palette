...
async function proxyRequest(req, res, config, path, label, runtime) {
	...
	try {
		...
		if (error?.code === "REQUEST_BODY_TOO_LARGE") writeJson(res, 413, {
			ok: false,
			error: {
				code: error.code,
		...
		else if (!req.aborted && !res.writableEnded) writeJson(res, 400, {
			ok: false,
	...
	writeJson(res, errorCode(lastFailure), {
		ok: false,
		error: { message: `${label} relay failed: ${errorMessage(lastFailure)}` }
	});
}
