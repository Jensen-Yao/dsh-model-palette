...
function registerGatewayRecovery(ctx, config = {}) {
...
function createGatewayRecoveryHandler(ctx, config = {}) {
...
function createBaiRelayHandler(rawConfig = {}) {
	const config = resolveConfig(rawConfig);
	return async (req, res) => {
		if (!isLoopbackRequest(req) || hasForwardedHeader(req)) {
			writeJson(res, 403, {
				ok: false,
				error: { message: "B.AI relay is available only to direct loopback requests" }
			});
			return;
		}
		let parsed;
		...
		const relativePath = parsed.pathname.slice(28);
		if (!relativePath.startsWith("/v1/")) {
			writeJson(res, 404, {
				ok: false,
				error: "B.AI relay accepts only /v1/* paths"
			});
			return;
		}
...
function resolveConfig(rawConfig) {
	...
	const upstreamHost = typeof value.upstreamHost === "string" && /^[a-z0-9.-]+$/iu.test(value.upstreamHost) ? value.upstreamHost : BAI_UPSTREAM_HOST;
	const hostHeader = typeof value.hostHeader === "string" && /^[a-z0-9.-]+$/iu.test(value.hostHeader) ? value.hostHeader : BAI_HOST_HEADER;
	const timeoutMs = value.timeoutMs === void 0 ? DEFAULT_TIMEOUT_MS : value.timeoutMs;
	if (!Number.isInteger(timeoutMs) || timeoutMs < 1e3 || timeoutMs > 9e5) throw new Error("dsh-model-palette: baiRelay.timeoutMs must be an integer from 1000 to 900000");
	return {
	...
	await new Promise((resolve) => {
		...
		const fail = (error) => {
			...
			if (!res.headersSent) writeJson(res, errorCode(error), {
				ok: false,
				error: { message: `B.AI relay failed: ${errorMessage(error)}` }
			});
