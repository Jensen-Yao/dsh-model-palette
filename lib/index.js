...
async function validateApiKey(ctx, req, res) {
	try {
		...
		writeJson(res, 200, {
			ok: true,
			value: await validateCredential(baseURL, protocol, model, apiKey)
		});
	} catch (error) {
		writeJson(res, 400, {
			ok: false,
...
async function validateCredential(baseURL, protocol, model, apiKey) {
	...
	if ("error" in catalogResponse) return {
		protocol,
		model,
		status: "unknown",
		checkedBy: "models",
		message: errorMessage(catalogResponse.error, apiKey)
	};
	if (catalogResponse.ok) return {
		...
		message: `HTTP ${catalogResponse.status}: model catalog accepted the API key`
	};
	if (![
		404,
		405,
		501
	].includes(catalogResponse.status)) return validationResult(protocol, model, classifyApiKeyStatus(catalogResponse.status), "models", catalogResponse.status, await responseMessage(catalogResponse, apiKey));
	if (model === void 0) return {
		protocol,
		status: "unknown",
		checkedBy: "models",
		httpStatus: catalogResponse.status,
		message: `HTTP ${catalogResponse.status}: model catalog is unavailable; select a model for a request test`
	};
...
async function validateModelRequest(baseURL, protocol, model, apiKey) {
	...
	try {
		...
		return validationResult(protocol, model, response.ok ? "valid" : classifyApiKeyStatus(response.status), "request", response.status, response.ok ? `HTTP ${response.status}: minimal model request succeeded` : await responseMessage(response, apiKey));
	} catch (error) {
		return {
			protocol,
			model,
